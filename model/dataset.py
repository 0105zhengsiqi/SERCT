# Code based on https://github.com/HoseinAzad/Transformer-based-SER
...
def collate_fn_padd(batch):
    ...
    # max_len = max([len(speech) for speech in speeches])
    # speeches = np.vstack([np.pad(speech, (0, max_len - len(speech)), 'constant', constant_values=0) for speech in speeches])
    max_len = 144000
    speeches = np.vstack(
        [
