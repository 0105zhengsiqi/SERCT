from cnn_bilstm import TemporalCNNBiLSTM
...
def trainModel(
    ...
    print('Loading model ...')
    model = TemporalCNNBiLSTM(num_classes)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        with open("exps/cnn_bilstm/log.txt", "a") as f:
            f.write(
                f'Train Loss = {train_loss:.4f} / Test Loss = {test_loss:.4f} / Test WA = {test_wa:.4f} / Test UA = {test_ua:.4f} / Test WF1 = {test_wf1:.4f}\n'
            )

        if (epoch + 1) % save_epoch == 0:
            torch.save(model, f'ckpts/cnn-bilstm/cnn-bilstm-{dataset_name}-{epoch + 1}.pt')

    torch.save(model, f'ckpts/cnn-bilstm/cnn-bilstm-{dataset_name}-last.pt')
